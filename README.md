...
# Profile 
Software developer engineer with 7+ years of software development experience. Have led the development process of a wide variety of solutions for multiple companies in the last years to support existing critical business operations. Enthusiast team player focused on outcomes that internal/external customers can achieve throughout the products/services provided. Open for feedback and willing to share with others from my experiences. Willing to take challenges that will help the team/company and also my professional development.
...
* Setup the AWS infrastructure for a Retool system that consisted in several ECS tasks, a backend database in a way that is compliant with security requirements (like the access through corporate VPN only) according to requirements. 
* Database design and implementation with Postgres according to functional requirements, reviews, integration and sync across environments.
* Use of infrastructure as code techniques using terraform + atlantis following best practices given by the SRE team.
* Implementation of features that reduces the network transfer outside AWS by 50+% by applying user friendly work flows. (query optimization, pagination, caching, among others)
* Database monitoring using the AWS capabilities/dashboard.
* Retool application development based on functional and nonfunctional requirements. This includes custom components that were required for very specific business needs. In which already created components could not be used.
* Participated in multiple user groups sessions to go over the changes, upcoming features and proposals, giving and receiving feedback and demoing the current state of the system
* Worked in several observability features using Grafana. 5+ dashboards with ~8 charts each based on the collected logs that gives a better visibility of what is happening with the different systems in near real time to help the support process, as well as getting notified about possible issues currently happening.
...
* Technical documentation and user guides writing, covering most popular use cases and relevant building blocks.
* REST API implementation using .NET 6 using best practices on code maintainability and testability.
* User test definitions using ZScaler tool
